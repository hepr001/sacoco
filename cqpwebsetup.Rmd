...
If you are reading this, you probably come from the [SaCoCo tutorial](index.html#set_up_a_corpus_in_cqpweb), you have access to a CQPweb installation as administrator, and you want to encode a corpus. We document two approaches below:
...
- root access to the server where CQPweb lives

### Encode the corpus for the CWB

The first thing we need to do is to encode the corpus. This process will create a number of files that will enable us to use the CQP language to query the corpus.

Once we have the texts in VRT format, encoding the corpus for the CWB is relatively easy.

Check that you have the corpus workbench installed on the computer, if not, download it and follow these [instructions](http://cwb.sourceforge.net/download.php). We compiled from source version 3.4.8.
...
Once you have the data you have to upload the file to the server where CQPweb is installed. In our case, this is the machine `fedora.clarin-d.uni-saarland.de`.
...
We set access to this corpus as open for everybody:
...
Let's assume that you have administrator access to a CQPweb installation. In the following lines, we will guide you through the process of setting up the corpus.
...
- Manage privileges: scroll to end of page and generate default privileges for the corpus; then select "Manage group grants", scroll to end of page and select a group and grant it privileges of that particular corpus (normally normal privileges are chosen)
